...
constexpr auto KEY_TRANSLATE_VERSION = u8"Version";
...
public:
	Window() : QDialog(nullptr, Qt::WindowMinMaxButtonsHint)
	{
		...
		i = -1;
		if (settings.contains(KEY_TRANSLATE_VERSION)) i = translateVersionCombo->findText(settings.value(KEY_TRANSLATE_VERSION).toString());
		if (i < 0) i = 0;
...
private:
	...
	{
		settings.setValue(KEY_TRANSLATE_VERSION, S(tlp->translateVersion = S(version)));
	}
} window;
...
TEST(
	{
		assert(Translate(L"こんにちは", { L"English (American)", L"?", L"free", L"" }).second.find(L"ello") == 1 || strstr(TRANSLATION_PROVIDER, "DevTools"));
	}
);
