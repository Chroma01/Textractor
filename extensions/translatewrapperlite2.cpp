...
namespace {
    ...
    void SaveCache() {
        ...
        for (const auto &[sentence, translation]: translationCache.Acquire().contents)
            allTranslations.append(L"|SENTENCE|").append(sentence).append(L"|TRANSLATION|").append(translation).append(
                L"|END|\r\n");
        std::ofstream(CacheFile(), std::ios::binary | std::ios::trunc).write(
            (const char *) allTranslations.c_str(), allTranslations.size() * sizeof(wchar_t));
    }
...
public:
    Window() : QDialog(nullptr, Qt::WindowMinMaxButtonsHint) {
        ...
        i = -1;
        if (settings.contains(KEY_TRANSLATE_FROM)) i = translateFromCombo->findText(
                                                   settings.value(KEY_TRANSLATE_FROM).toString());
        if (i < 0) i = 0;
        ...
        for (auto [value, label, keylabel]: Array<bool &, const char *, const char *>{
            ...
            connect(checkBox, &QCheckBox::clicked, [keylabel, &value](bool checked) {
                settings.setValue(keylabel, value = checked);
            });
        }
        for (auto [value, label, keylabel]: Array<int &, const char *, const char *>{
            ...
            connect(spinBox, qOverload<int>(&QSpinBox::valueChanged), [keylabel, &value](int newValue) {
                settings.setValue(keylabel, value = newValue);
            });
        }
...
TEST(
    {
    assert(Translate(L"こんにちは", { L"english", L"?"}).second.find(L"ello") == 1 || strstr(TRANSLATION_PROVIDER, "DevTools"
    ));
